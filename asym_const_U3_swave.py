...
processedasymfolder = '/processed_data/'+uniquefolder+'asym_swave_U/'
AsymSavename = os.getcwd()+processedasymfolder+run_num+'_swave_U'
logger.add(basedir+"/processed_data/" + uniquefolder + '0_ErrorLog_'+run_start+'_'+run_end+'_swave_U.txt', delay = False)
...
if target == 'La': #######################################################CHANGED FOR S WAVE
    vparamsfileloc = basedir+'/processed_data/'+'s_vparams_La.h5'
elif target == 'Pr':
...
def voigt_fitting_c(ogbin_res_reg, aftrebin_res_reg,xs,ys): ## currently xdata which uses ogbin_res_reg is defined outside of function. But keeping it as an input for now.
    ...
    for seq in range(0, len(ys)): ## number of sequences, usually 13
        ...
        fit_params = popt[0]
        fit_errs = np.sqrt(np.diagonal(pcov))[0]
...
def voigt_fitting(ogbin_res_reg, aftrebin_res_reg,xs,ys):
    ...
    for seq in range(0, len(ys)): ## number of sequences, usually 13
        ...
        fit_curves.append(fitted_curve)
        fit_params = popt
        fit_errs = np.sqrt(np.diagonal(pcov))
