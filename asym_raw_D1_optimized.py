...
def asymraw(ON_arr, OFF_arr):
    ...
    for seq in range(0, len(ON_arr)): ## number of sequences
        for j in range(len(ON_arr[seq])):
            denom = ON_arr[seq][j] + OFF_arr[seq][j]
            if denom != 0:
                tempasym[j] += (ON_arr[seq][j] - OFF_arr[seq][j]) / denom
    return tempasym
...
asym_int = np.full((len(ON_sums)), -1.0, dtype=np.float64)  ## 1 Asym for each channel, and its error
asym_ch_err = np.full((len(ON_sums),2), -1.0, dtype=np.float64)  ## 1 Asym for each channel, and its error
asym_ch_err_varied = np.full((len(ON_sums),2), -1.0, dtype=np.float64)  ## 1 Asym for each channel, and its error
v_sigmas = np.full((len(ON_sums),3), -1.0, dtype=np.float64)
v_gammas = np.full((len(ON_sums),3), -1.0, dtype=np.float64)
v_reslocs = np.full((len(ON_sums),3), -1.0, dtype=np.float64)
fwhm_bins = np.full((len(ON_sums),2), -1, dtype=np.int32)
