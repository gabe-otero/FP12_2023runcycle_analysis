...
def voigt_c(x, amp, c): ## takes sig, gamma, res_loc as constants from file  # ** changed 05.02.26 for >0 constraint and Vfit + constant
    fit = voigt_profile(x-xloc[0], sig[0], gam[0])*amp + c  # ** changed 05.02.26 for >0 constraint and Vfit + constant
    return fit
...
def voigt_fitting_c(ogbin_res_reg, aftrebin_res_reg,xs,ys):
    ...
    for seq in range(0, len(ys)): ## number of sequences, usually 13
        ...
        popt, pcov = curve_fit(
            voigt_c, xs, ydata,
            p0=[np.max(ydata), 0],  # ** changed 05.02.26 for >0 constraint and Vfit + constant
            bounds = ([0, -np.inf], [np.inf, np.inf])  # ** changed 05.02.26 for >0 constraint and Vfit + constant
        )
        fitted_curve = voigt_c(xs, popt[0], popt[1])  # ** changed 05.02.26 for >0 constraint and Vfit + constant
        fit_curves.append(fitted_curve)

        amp_err = np.sqrt(np.diagonal(pcov))[0]
        c_err = np.sqrt(np.diagonal(pcov))[1]

        parameters.append([popt[0], amp_err])  # ** changed 05.02.26 for >0 constraint and Vfit + constant
        constants.append([popt[1], c_err])     # ** changed 05.02.26 for >0 constraint and Vfit + constant
...
def voigt2(x, s, g, amp, c):  # ** changed 05.02.26 for >0 constraint and Vfit + constant
    fit = voigt_profile(x-xloc[0], s, g)*amp + c  # ** changed 05.02.26 for >0 constraint and Vfit + constant
    return fit
...
def voigt_fitting(ogbin_res_reg, aftrebin_res_reg,xs,ys):
    ...
    for seq in range(0, len(ys)): ## number of sequences, usually 13
        ...
        popt, pcov = curve_fit(
            voigt2, xs, ydata,
            p0=[s_guess,g_guess,a_guess,0],  # ** changed
            bounds = ([s_guess-s_1std,g_guess-g_1std,0,-np.inf], [s_guess+s_1std,g_guess+g_1std,np.inf,np.inf])  # ** changed
        )
        fitted_curve = voigt2(xs, popt[0],popt[1],popt[2],popt[3])  # ** changed
        fit_curves.append(fitted_curve)

        fit_params = np.array([popt[0], popt[1], popt[2]])  # ** changed 05.02.26 for >0 constraint and Vfit + constant
        fit_errs = np.sqrt(np.diagonal(pcov))[:3]          # ** changed 05.02.26 for >0 constraint and Vfit + constant
        const_val = popt[3]                                # ** changed 05.02.26 for >0 constraint and Vfit + constant
        const_err = np.sqrt(np.diagonal(pcov))[3]          # ** changed 05.02.26 for >0 constraint and Vfit + constant

        parameters.append([fit_params,fit_errs])
        constants.append([const_val, const_err])           # ** changed 05.02.26 for >0 constraint and Vfit + constant
    return fit_curves, parameters, constants
...
# ** changed 05.02.26 for >0 constraint and Vfit + constant
...
with h5py.File(AsymSavename+'.h5', 'w') as hdf5_file:
    ...
    for i in range(0,len(asym_ch_err)): 
        ...
        ON_subgrp.create_dataset('parameters_varied', data=ON_vfit_params_varied[i])  # ** changed 05.02.26 for >0 constraint and Vfit + constant
        OFF_subgrp.create_dataset('parameters_varied', data=OFF_vfit_params_varied[i])  # ** changed 05.02.26 for >0 constraint and Vfit + constant

        ON_subgrp.create_dataset('constant_varied', data=ON_vfit_const_varied[i])  # ** changed 05.02.26 for >0 constraint and Vfit + constant
        OFF_subgrp.create_dataset('constant_varied', data=OFF_vfit_const_varied[i])  # ** changed 05.02.26 for >0 constraint and Vfit + constant
