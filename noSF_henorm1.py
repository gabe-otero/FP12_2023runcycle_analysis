...
chan_enab = [25]  ## no SF channels for this one
...
logger.add("F:/LANL/SF_Norm_files/" + uniquefolder + '0_ErrorLog_'+run_start+'_'+run_end+'.txt', delay = True)
...
print('saving norm information to ' + savefilename)
end = time.time()
# print(end-start)
# print(read_data)
...
ys_arrHe, ETTT_arr, eventcount_arr  = dataread(read_data, np.array([25]), fileLength, numSamples) ##hardcoded channel 25 for He. no SF channels
...
# Calculate the time difference between each event within a file - used to check for dropping pulses. It seems that if we make the record window 49.152 ms long, we miss every other pulse (at 20 Hz). 
# This is not that surprising - we presumably will not need a lot of data (or any) with the full 50 ms time window.
...
## removed below - only doing 3He normalization.
# In[7]:
...
######################## below sets code only for 3He norm while keeping structure of SF organization ###################
transitions = ['bogus']
AllSwitches = ['bogus']
