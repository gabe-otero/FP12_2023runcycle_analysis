...
processedasymfolder = '/processed_data/'+uniquefolder+'asym_raw_D/'  ## CHAT raw-asym output folder
...
AsymSavename = os.getcwd()+processedasymfolder+run_num+'_raw_D_optimized'  ## CHAT raw-asym output filename
# AsymSavename_bg = os.getcwd()+processedasymfolder_bg+run_num+'_asym_bg_D' ## maybe canget rid of this
logger.add(basedir+"/processed_data/" + uniquefolder + '0_ErrorLog_'+run_start+'_'+run_end+'_raw_D_optimized.txt', delay = False)  ## CHAT raw-asym log
...
# CHAT: voigt constants are not used in the raw-asym pipeline
# if target == 'La':
...
vparamsfileloc = 'no_fit'  ## CHAT no Voigt fit constants for raw-asym output

print("Target is " + target + ', voigt fit params disabled; raw asymmetry only')
# In[3]:
...
@njit
def find_offset_numba(ys, thresharr):
    n_ch = len(ys)
    n_pulses = len(ys[0])
    n_samples = len(ys[0][0])
    xCrosses = np.zeros((n_ch, n_pulses), dtype=np.int32)
    offset = np.zeros((n_ch, n_pulses), dtype=np.int32)
    modeCrosses = np.zeros(n_ch, dtype=np.int32)
    missing_6li = 0
    ...
    for i in range(n_ch - 1, -1, -1):
        ...
        for p in range(n_pulses):
            ...
            for j in range(n_samples):
                if ys[i][p][j] > thresharr[i]:
                    xing = j
                    found = 1
                    break
            if found == 0 and i == n_ch - 1:
                missing_6li = 1
            xCrosses[i][p] = xing
            counts[xing] += 1
        mode_val = 0
        ...
        for j in range(1, n_samples):
            if counts[j] > best:
                best = counts[j]
                mode_val = j
        modeCrosses[i] = mode_val

    for i in range(n_ch):
        for p in range(n_pulses):
            offset[i][p] = modeCrosses[n_ch - 1] - xCrosses[i][p]

    return offset, xCrosses, modeCrosses, missing_6li

offset, xCrosses, modeCrosses, missing_6li = find_offset_numba(ys_basesub, threshold_array)
if missing_6li == 1:
...
# CHAT pad_numba and roll_numba removed; direct indexed alignment is faster and uses less memory
...
# CHAT direct indexed alignment avoids padding, rolling, and an extra temporary array
@njit
def align_cut_numba(ys, xs_arr, extendedr, offsets, norms):
    n_pulses = len(ys)
    n_samples = len(ys[0])
    new_length = n_samples - 2*extendedr - 2
    tempys_cut = np.zeros((n_pulses, new_length), dtype=np.float64)
    for p in range(n_pulses):
        shift = offsets[p]
        start_idx = extendedr + 1 - shift
        for j in range(new_length):
            src_idx = start_idx + j
            if src_idx >= 0 and src_idx < n_samples:
                tempys_cut[p][j] = ys[p][src_idx] / norms[p]
            else:
                tempys_cut[p][j] = 0.0
    x_cut_amt = extendedr + 1
...
try:
    for i in range(len(ys_basesub)):
        ys_cut[i], xs_cut[i] = align_cut_numba(ys_basesub[i], xs[i], extendedRange, offset[i], HeNorms)
except Exception as e:
...
# #print('extended range index for checkpulse: '+str(np.argmax((ys_cut[0][checkp]*HeNorms[checkp])> 2000)))
...
# CHAT precompute ON/OFF pulse ranges so we can sum slices instead of looping pulse-by-pulse
seq_on_ranges = []
seq_off_ranges = []
for seq in range(len(sequence[0])):
    on_ranges = []
    off_ranges = []
    for state in range(len(sequence[1][0])):
        s = sequence[1][seq][state]
        state_range = sequence[2][seq][state]
        if s==0 or s==3 or s==5 or s==6: ## these are ON states
            on_ranges.append((state_range[0], state_range[1]))
        if s==1 or s==2 or s==4 or s==7: ## these are OFF states
            off_ranges.append((state_range[0], state_range[1]))
    seq_on_ranges.append(on_ranges)
    seq_off_ranges.append(off_ranges)

# CHAT pulse-range version is much faster than pulse-by-pulse accumulation and still preserves the sequence-level asymmetry
## CHAT add_pulse_fast keeps sequence-level sums but avoids pulse-by-pulse Python accumulation
def add_pulse_fast(ys, seq_on_ranges, seq_off_ranges):
    temp_ON = np.zeros((len(seq_on_ranges), len(ys[0])), dtype=np.float64)
    temp_OFF = np.zeros((len(seq_off_ranges), len(ys[0])), dtype=np.float64)
    for seq in range(len(seq_on_ranges)):
        for start_stop in seq_on_ranges[seq]:
            temp_ON[seq] += np.add.reduce(ys[start_stop[0]:start_stop[1]], axis=0)
        for start_stop in seq_off_ranges[seq]:
            temp_OFF[seq] += np.add.reduce(ys[start_stop[0]:start_stop[1]], axis=0)
    return temp_ON, temp_OFF

# CHAT compute raw asymmetry sequence-by-sequence so the small effect does not wash out
asym_raw = np.zeros((len(ys_cut), len(ys_cut[0][0])), dtype=np.float64) ## 1 Asym for each channel, not for each sequence (can change?)
for i in range(len(ys_cut)):
    ON_tmp, OFF_tmp = add_pulse_fast(ys_cut[i], seq_on_ranges, seq_off_ranges)
    den = ON_tmp + OFF_tmp
    seq_asym = np.divide(ON_tmp - OFF_tmp, den, out=np.zeros_like(den), where=den != 0)
    asym_raw[i] = np.sum(seq_asym, axis=0)
    del ON_tmp, OFF_tmp, den, seq_asym
...
# CHAT ### load in voigt values and regions
# CHAT v_sigmas = []
# CHAT v_gammas = []
# CHAT v_reslocs = []
# CHAT with h5py.File(vparamsfileloc, 'r') as f: ## new arr_sizer
# CHAT     channels_all = list(f.keys())
# CHAT     # print(np.asarray(f.attrs))
# CHAT     # print(np.asarray(f[channels_all[0]].attrs))
# CHAT     bg_reg1 = f.attrs.get('bg_reg_bef')
# CHAT     bg_reg2 = f.attrs.get('bg_reg_aft')
# CHAT     res_reg = f.attrs.get('res_reg')
# CHAT     for i in range(chan_enab[0], chan_enab[-1]):
# CHAT #         print(i)
# CHAT         v_sigmas.append(f[channels_all[i]].attrs.get('sigma_[mean,mode,std]')) ## each of these for each channel
# CHAT         v_gammas.append(f[channels_all[i]].attrs.get('gamma_[mean,mode,std]'))
# CHAT         v_reslocs.append(f[channels_all[i]].attrs.get('res_loc_[mean,mode,std]')) ## in ms, not bins
# CHAT     f.close()

# CHAT v_sigmas = np.asarray(v_sigmas)
# CHAT v_gammas = np.asarray(v_gammas)
# CHAT v_reslocs = np.asarray(v_reslocs)
# CHAT fullrange = bg_reg2[1]-bg_reg1[0]

# CHAT ## one f_g/l/v for each channel
# CHAT f_g = 2*(v_sigmas[:,0])*np.sqrt(2*np.log(2)) ## Gaussian FWHM
# CHAT f_l = 2*(v_gammas[:,0]) ##Lorentzian FWHM
# CHAT f_v = 0.5343*f_l + np.sqrt(0.2169*f_l**2+f_g**2)  ## this is the FWHM from wikipedia. # one FWHM value for each channel

# CHAT # In[18]:

# CHAT from scipy.special import voigt_profile
# CHAT from scipy.optimize import curve_fit
# CHAT ## change to 2nd order poly in cleaned up version
# CHAT start = time.time()

# CHAT def bg_fitsubtract(bef_res_reg, aft_res_reg, ys, order): ## before/after resonance region [start:end] respectively, ys[ch] to fit
# CHAT     binstot = aft_res_reg[1]-bef_res_reg[0]  ## total number of bins in whole region
# CHAT     x1 = np.arange(bef_res_reg[0], bef_res_reg[1],1)
# CHAT     x2 = np.arange(aft_res_reg[0], aft_res_reg[1],1)
# CHAT     x = np.append(x1,x2)
# CHAT     fullx = np.arange(bef_res_reg[0], aft_res_reg[1],1) ## an array of every x bin in entire region
# CHAT     ys_bgsub = []
# CHAT     for seq in range(0, len(ys)): ## number of sequences, usually 13
# CHAT         fitdata1 = ys[seq][bef_res_reg[0]: bef_res_reg[1]]
# CHAT         fitdata2 = ys[seq][aft_res_reg[0]: aft_res_reg[1]]
# CHAT         datasplice = np.append(fitdata1, fitdata2)
# CHAT         y = datasplice
        
# CHAT         coeffs, errs = polyN_fit_errors(x, y,order) # use new fit functions 
# CHAT     #     print("Fitted Coefficients:", coeffs)
# CHAT     #     print('Uncertainty ', errs)
# CHAT         y_fit = polyN_predict(x, coeffs) ## run the function with the coeff you just found
# CHAT         fullpoly_y2 = polyN_predict(fullx, coeffs)

# CHAT         bgsubtracted = ys[seq][bef_res_reg[0]:aft_res_reg[1]] - fullpoly_y2  ## subtracts RealData-BackgroundFit
# CHAT         ys_bgsub.append(bgsubtracted)
# CHAT     return ys_bgsub

# CHAT # background subtraction currently only for NaI detectors
# CHAT num_fittingchs = len(ON_sums)-1  ## removes the Li detector, assumes it is there
# CHAT ON_bgsub = np.zeros((num_fittingchs,len(ON_sums[0]),fullrange), dtype = np.float64) ## channels, sequences, range of bg_ subtraction
# CHAT OFF_bgsub = np.zeros((num_fittingchs,len(ON_sums[0]),fullrange), dtype = np.float64) ## channels, sequences, range of bg_ subtraction
# CHAT fitorder = 4
# CHAT for i in range(0, len(ON_sums)-1):
# CHAT     if chan_enab[i] == 24:
# CHAT         emessage = ('bg_ fit does not work for 6Li yet')
# CHAT         logger.error('run '+run_num + emessage)
# CHAT         raise Exception(emessage)
# CHAT     ON_bgsub[i]  = bg_fitsubtract(bg_reg1,bg_reg2, ON_sums[i], fitorder)
# CHAT     OFF_bgsub[i] = bg_fitsubtract(bg_reg1,bg_reg2, OFF_sums[i], fitorder)

# CHAT # end = time.time()
# CHAT # print('bg_ fitting time: ' + str(end-start))  

# CHAT # In[19]:
# CHAT # now new voigt, with constants

# CHAT ## define new voigt functions using constants loaded in
# CHAT def voigt_c(x, amp): ## takes sig, gamma, res_loc as constants from file
# CHAT     fit = voigt_profile(x-xloc[0], sig[0], gam[0])*amp ## [0] indicates the mean value of param, not mode or std
# CHAT     return fit

# CHAT def voigt_fitting_c(ogbin_res_reg, aftrebin_res_reg,xs,ys): ## currently xdata which uses ogbin_res_reg is defined outside of function. But keeping it as an input for now.
# CHAT     fit_curves = [] ## now this function uses the res region as opposed to the not-background-region
# CHAT     parameters = []
# CHAT     for seq in range(0, len(ys)): ## number of sequences, usually 13
# CHAT         ydata = ys[seq][aftrebin_res_reg[0]:aftrebin_res_reg[1]]
# CHAT         popt, pcov = curve_fit(voigt_c, xs, ydata,p0=[np.max(ydata)], bounds = ([-np.inf], [np.inf]))
# CHAT         fitted_curve = voigt_c(xs, popt[0],) ## sigma, gamma, xshift (res. center), amp. related thing 
# CHAT         # popt, pcov = curve_fit(voigt2, xs, ydata, bounds = ([0,0,0,-np.inf], [np.inf,np.inf,np.inf, np.inf]))  ## these are from the old function
# CHAT         # fitted_curve = voigt2(xs, popt[0],popt[1],popt[2],popt[3],) ## sigma, gamma, xshift (res. center), amp. related thing
# CHAT         fit_curves.append(fitted_curve)
# CHAT         fit_params = popt[0]
# CHAT         fit_errs = np.sqrt(np.diagonal(pcov))[0]
# CHAT         parameters.append([fit_params,fit_errs])  ## only return the amplitude parameter now
# CHAT     return fit_curves, parameters

# CHAT ## go back to original voigt fitting but this time with suggested guesses and bounds on possible param

# CHAT def voigt2(x, s, g, amp):
# CHAT     fit = voigt_profile(x-xloc[0], s, g)*amp
# CHAT     return fit

# CHAT def voigt_fitting(ogbin_res_reg, aftrebin_res_reg,xs,ys):
# CHAT     fit_curves = []
# CHAT     parameters = []
# CHAT     for seq in range(0, len(ys)): ## number of sequences, usually 13
# CHAT         ydata = ys[seq][aftrebin_res_reg[0]:aftrebin_res_reg[1]]
# CHAT         a_guess, s_guess, g_guess = np.max(ydata), sig[0], gam[0]
# CHAT         s_1std, g_1std = sig[2]/2, gam[2]/2  ## allow for half of the std
# CHAT         ## set the guesses as the loaded values, and the bounds on sigma,gamma as +/- 1 std
# CHAT         popt, pcov = curve_fit(voigt2, xs, ydata,p0=[s_guess,g_guess,a_guess], bounds = ([s_guess-s_1std,g_guess-g_1std,-np.inf], [s_guess+s_1std,g_guess+g_1std,np.inf]))
# CHAT         fitted_curve = voigt2(xs, popt[0],popt[1],popt[2]) ## sigma, gamma, amp. related thing
# CHAT         fit_curves.append(fitted_curve)
# CHAT         fit_params = popt
# CHAT         fit_errs = np.sqrt(np.diagonal(pcov))
# CHAT         parameters.append([fit_params,fit_errs])
# CHAT     return fit_curves, parameters

# CHAT # In[21]:

# CHAT if res_reg[0] < bg_reg1[1] or res_reg[1]>bg_reg2[0]:
# CHAT     emessage = ('Declared Background region and Resonance region have overlapping fitting regions')
# CHAT     logger.error('run '+run_num + emessage)
# CHAT     raise Exception(emessage)
...
# CHAT res_size    = res_reg[1]-res_reg[0] 
# CHAT newresstart = res_reg[0]-bg_reg1[0]
# CHAT newresend   = (res_reg[1]-res_reg[0])+newresstart
# CHAT new_res_reg = [newresstart, newresend]  ## this just realigns in terms of bins. Probably a nicer way to do it.

# CHAT xdata = xs_cut[0][res_reg[0]:res_reg[1]]*1e-6  ## just change all xs to ms and one array

# CHAT ON_vfit = np.zeros((len(ON_bgsub),len(ON_bgsub[0]),res_size), dtype = np.float64) ## channels, sequences, range of V_ subtraction
# CHAT OFF_vfit = np.zeros((len(ON_bgsub),len(ON_bgsub[0]),res_size), dtype = np.float64) ## channels, sequences, range of V_ subtraction
# CHAT ON_vfit_params  = np.zeros((len(ON_bgsub),len(ON_bgsub[0]),2), dtype = np.float64) ## channels, sequences, [amp thing, amp thing err]
# CHAT OFF_vfit_params = np.zeros((len(ON_bgsub),len(ON_bgsub[0]),2), dtype = np.float64)

# CHAT sig_ch = np.zeros((len(ON_vfit),2), dtype=np.float64) ## 1 sigma +/- error for each channel
# CHAT gam_ch = np.zeros((len(ON_vfit),2), dtype=np.float64) ## 1 gamma +/- error for each channel
# CHAT ON_vfit_params_varied  = np.zeros((len(ON_bgsub),len(ON_bgsub[0]),2,3), dtype = np.float64) ## channels, sequences,[params, param_errs], [sigma, gamma, amp thing] NO xSHIFT
# CHAT OFF_vfit_params_varied = np.zeros((len(ON_bgsub),len(ON_bgsub[0]),2,3), dtype = np.float64)

# CHAT for ch in range(0, len(ON_bgsub)):
# CHAT     sig  = v_sigmas[ch] ## the definitions of these changes per channel for func voigt_c
# CHAT     gam  = v_gammas[ch]
# CHAT     xloc = v_reslocs[ch]
# CHAT     if chan_enab[ch] == 24:
# CHAT         emessage = ('bg_ fit does not work for 6Li yet')
# CHAT         logger.error('run '+run_num + emessage)
# CHAT         raise Exception(emessage)
# CHAT     try:
# CHAT         ON_vfit[ch],  ON_vfit_params[ch]  = voigt_fitting_c(res_reg,new_res_reg,xdata, ON_bgsub[ch])
# CHAT         OFF_vfit[ch], OFF_vfit_params[ch] = voigt_fitting_c(res_reg,new_res_reg,xdata, OFF_bgsub[ch])
# CHAT         trash, ON_vfit_params_varied[ch]  = voigt_fitting(res_reg,new_res_reg,xdata, ON_bgsub[ch])  ## these 2 are the original w varying sig,gam
# CHAT         trash, OFF_vfit_params_varied[ch] = voigt_fitting(res_reg,new_res_reg,xdata, OFF_bgsub[ch])
# CHAT     except Exception as e:
# CHAT         logger.error('run '+run_num +' ch '+ str(chan_enab[ch])+' failed during Voigt fitting')
# CHAT         logger.exception(e)

# CHAT end = time.time()
# CHAT print('bg_ fitting time: ' + str(end-start)) 

# CHAT # In[23]:
# CHAT # sum up all pulses, and then calculate asymmetry using "amplitude"

# CHAT # ## dont think need this now that there is addpulses program
# CHAT # all_ONOFF = np.zeros((len(ON_sums),2, len(ON_sums[0][0])), dtype=np.float64) ## all summed ON[0] and OFF[1] pulses for each channel, not each sequence

# CHAT # def all_onoff(ON_arr, OFF_arr):
# CHAT #     tempallON = np.zeros((len(ON_arr[0])), dtype=np.float64)
# CHAT #     tempallOFF = np.zeros((len(ON_arr[0])), dtype=np.float64)
# CHAT #     for seq in range(0, len(ON_arr)): ## number of sequences
# CHAT #         tempallON = np.add(ON_arr[seq],tempallON)
# CHAT #         tempallOFF = np.add(OFF_arr[seq],tempallOFF)
# CHAT #     return tempallON, tempallOFF

# CHAT # for i in range(len(ON_sums)):
# CHAT #     all_ONOFF[i][0], all_ONOFF[i][1] = all_onoff(ON_sums[i], OFF_sums[i]) ## channel, ON[0] or OFF[1], summed wave

# CHAT # new asym integrating over a curve and using FWHM of voigt function

# CHAT start = time.time()

# CHAT xs_full_res = xs_cut[0][bg_reg1[0]:bg_reg2[1]]*1e-6 ## because this is in ms an not bins, shouldn't matter if it is [bg bin1, bg bin2] or [res bin1, res bin2], but output bins will change accordingly
# CHAT fwhm_bins = []
# CHAT for ch in range(0, len(f_v)):
# CHAT     fwhm_bin_left = np.where(xs_full_res>=v_reslocs[ch][0]-f_v[ch]/2)[0][0]
# CHAT     fwhm_bin_right = np.where(xs_full_res>=v_reslocs[ch][0]+f_v[ch]/2)[0][0]
# CHAT     fwhm_bins.append([fwhm_bin_left, fwhm_bin_right])
# CHAT # print(fwhm_bins)

# CHAT asym_int = np.zeros((len(ON_vfit)), dtype=np.float64) ## 1 Asym for each channel, and its error
# CHAT def asym_integral(ON_data, OFF_data, integral_bins):  ## currently does not do errors...
# CHAT     tempasym = 0
# CHAT     temperr  = []
# CHAT     fwhm_l = integral_bins[0] # easier to read
# CHAT     fwhm_r = integral_bins[1]
# CHAT     for seq in range(0, len(ON_data)): ## number of sequences
# CHAT         A_plus = np.sum(ON_data[seq][fwhm_l:fwhm_r]) ## just makes things cleaner
# CHAT         A_min  = np.sum(OFF_data[seq][fwhm_l:fwhm_r])
# CHAT         seqasym = ((A_plus-A_min) / (A_plus+A_min))
# CHAT         # ON_err = 1/np.sqrt(A_plus) ## don't do hard version of error for now 05.02.25
# CHAT         # OFF_err = 1/np.sqrt(A_min)
# CHAT         tempasym = np.add(seqasym,tempasym)
# CHAT     return tempasym  ## don't normalize asymON_vfit_params this time...

# CHAT for i in range(len(ON_vfit)):
# CHAT     asym_int[i] = asym_integral(ON_bgsub[i], OFF_bgsub[i], fwhm_bins[i])  ## because sig,gam,xloc are the same for ON/OFF, using vfit curves gives same asymm as just using amp factor

# CHAT asym_raw = np.zeros((len(ON_sums), len(ON_sums[0][0])), dtype=np.float64) ## 1 Asym for each channel, not for each sequence (can change?)
# CHAT @njit
# CHAT def asymraw(ON_arr, OFF_arr):
# CHAT     tempasym = np.zeros((len(ON_arr[0])), dtype=np.float64)
# CHAT     for seq in range(0, len(ON_arr)): ## number of sequences
# CHAT         for j in range(len(ON_arr[seq])):
# CHAT             denom = ON_arr[seq][j] + OFF_arr[seq][j]
# CHAT             if denom != 0:
# CHAT                 tempasym[j] += (ON_arr[seq][j] - OFF_arr[seq][j]) / denom
# CHAT     return tempasym
# CHAT 
# CHAT for i in range(len(ON_sums)):
# CHAT     asym_raw[i] = asymraw(ON_sums[i], OFF_sums[i])
...
# CHAT for i in range(len(ON_vfit)):
# CHAT     sig_ch[i] = sum_param(ON_vfit_params_varied[i], OFF_vfit_params_varied[i], key='sigma')
# CHAT     gam_ch[i] = sum_param(ON_vfit_params_varied[i], OFF_vfit_params_varied[i], key='gamma') 
...
# CHAT raw-asym only: fit-related outputs replaced with -1 placeholders
asym_int = np.full((len(asym_raw)), -1.0, dtype=np.float64)  ## 1 Asym for each channel, and its error
asym_ch_err = np.full((len(asym_raw),2), -1.0, dtype=np.float64)  ## 1 Asym for each channel, and its error
asym_ch_err_varied = np.full((len(asym_raw),2), -1.0, dtype=np.float64)  ## 1 Asym for each channel, and its error
v_sigmas = np.full((len(asym_raw),3), -1.0, dtype=np.float64)
v_gammas = np.full((len(asym_raw),3), -1.0, dtype=np.float64)
v_reslocs = np.full((len(asym_raw),3), -1.0, dtype=np.float64)
fwhm_bins = np.full((len(asym_raw),2), -1, dtype=np.int32)
...
# CHAT raw-asym output block (same metadata, same channel groups, no ON/OFF or Voigt products)
# CHAT memory cleanup for raw-only pipeline
try:
    del ys_cut
except Exception:
    pass
...
with h5py.File(AsymSavename+'.h5', 'w') as hdf5_file:
    ...
    hdf5_file.attrs['processing_note'] = 'CHAT raw asymmetry only; Voigt-fit quantities set to -1; ON/OFF subgroups omitted'
    hdf5_file.attrs['voigt_fit_status'] = 'no_fit'
