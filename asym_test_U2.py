...
# AddedPulseSavename = processedpulsefolder+uniquefolder+run_num+'_pulsesadded_u'
# AsymSavename = processedasymfolder+uniquefolder+run_num+'_asym_u'
...
#                 tempadded_pulses[i] = added_pulses[i]+np.array(ys_cut[i][j])
            ...
            if state==2 or state==4 or state==5 or state==7: ## these are ON states
                temp_ONOFF[seq][0] = np.add(temp_ONOFF[seq][0],(tempadded_p[seq][state])) ## start with zeros, add to each iteratively
            if state==0 or state==1 or state==3 or state==6: ## these are OFF states
